...
//1) get MonthDay
//2) get "now" as LocalDate
//3) get year for "now" - int
//4) figure out if MonthDay is valid for year (if not, increment year... go to #4)
//5) get MonthDay + year for the LocalDate
//6) figure out if LocalDate is in future (if not, increment year... go to #4)
//7) we know the next occurrence (return)

    // todo: Implement the next() method by following the instructions below
        //Find next date occurrence (e.g. of a holiday)
    public static LocalDate next(MonthDay date){
        ...
        boolean isValidfForThisyear = date.isValidYear(year);
    ...
    // todo: create the daysTo() method
        // Find # of days till next occurrence.
